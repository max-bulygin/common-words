...
`thumbnail`, `thumb`, `thmb` — миниатюра, уменьшенное изображение

## Текст

`title`, `heading`, `headline` — заголовок

`caption` - подпись, пояснение. Обычно используется с изображениями

`subject` - заголовок письма или тема
...
`list`, `listing` — список, результат поисковой выдачи

`items` - группа элементов

`item`, `element` — элемент списка или группы элементов
...
`hero` - первый блок на странице, обычно занимает всю ширину экрана

## Раскладка

`wrapper`, `wrap`, `holder` — обёртка, обычно внешняя

`outer` - внешняя обёртка

`inner` — внутренняя обёртка

`container`, `shell` — контейнер

`grid` — раскладка (страницы или элемента) в виде сетки (обычно содержит в себе `row` и `col`)

`row`, `range` — контейнер в виде строки

`col`, `column`, `cell` — контейнер в виде столбца
...
`cta` - _call to action_ - кнопка, призывающая совершить действие на сайте

`control` — элемент управления, например, стрелки «Вперёд/назад» в фотогалерее, кнопки управления слайдером

`dropdown` — выпадающий список

`pick`, `picker` - выбор одного из вариантов

`trigger` - элемент вызывающий событие

`toggle` - тумблер, переключение между состояниями. Обычно имеет js обработчик события
...
_Сокращенные названия лучше использовать только в качестве модификаторов класса. Например:_
```html
<a href="#" class="button button--sm"></a>
```
## Декоративные элементы

`ribbon`, `stripe` - лента. Декоративный элемент для отображения дополнительной информации о товаре (скидка, акция, иногда цена)

`badge` - бейдж, значок

`tooltip`, `tip` — всплывающее подсказки на подобие аттрибута title

`outline` - обводка, граница элемента

`divider` - разделитель

`pill` - элемент с сильно скругленными углами, по форме напоминает пиллюлю
...
`features`, `benefits`, `advantages` — список основных особенностей товара, услуги
...
`portrait`, `vert` - расположение элемента или экрана при котором его высота больше его ширины

`landscape`, `horz` - расположение элемента или экрана при котором его ширина больше его высоты
...
`lastadded` - новый, недавно добавлен
