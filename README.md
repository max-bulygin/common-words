...
`picker` - выбор одного из вариантов, например `language-picker`

`trigger` - элемент вызывающий событие

`toggle` - тумблер, переключение между состояниями, например `menu-toggle`
...
`portrait`, `vert` - вертикальное расположение

`landscape`, `horz` - горизонтальное расположение
